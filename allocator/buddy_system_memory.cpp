#include "buddy_system_memory.h"
#include <cmath>

size_t buddy_system_memory::get_allocator_service_block_size() const
{
    return sizeof(size_t) + sizeof(memory*) + sizeof(logger*) + sizeof(memory::allocation_mode) + sizeof(void *);
}

size_t buddy_system_memory::get_allocated_memory_size() const
{
...
logger * buddy_system_memory::get_logger() const
{
    return *reinterpret_cast<logger**>(reinterpret_cast<unsigned char*>(_allocated_memory) + sizeof(size_t) + sizeof(memory*));
}

memory::allocation_mode buddy_system_memory::get_allocation_mode() const
{
...
void ** buddy_system_memory::get_first_available_block_ptr() const
{
...
void * buddy_system_memory::get_first_available_block() const
{
    return *get_first_available_block_ptr();
}

size_t buddy_system_memory::get_occupied_block_size(void * current_block) const
{
    return 1 << (*(reinterpret_cast<unsigned char*>(current_block) - 1) >> 1);
}

size_t buddy_system_memory::get_occupied_block_size_without_service_block(void * current_block) const
{
    return get_occupied_block_size(current_block) - get_occupied_block_service_block_size();
}

size_t buddy_system_memory::get_available_block_size(void * current_block) const
{
    return 1 << (*reinterpret_cast<unsigned char*>(current_block) >> 1);
}

void * buddy_system_memory::get_previous_available_block(void *current_block) const
{
    return *reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(current_block) + sizeof(unsigned char));
}

void * buddy_system_memory::get_next_available_block(void * current_block) const
{
    return *reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(current_block) + sizeof(unsigned char) + sizeof(void*));
}

size_t buddy_system_memory::get_available_block_service_block_size() const
{
    return sizeof(unsigned char) + sizeof(void*) + sizeof(void*);
}

size_t buddy_system_memory::get_occupied_block_service_block_size() const
{
    return sizeof(unsigned char);
}

bool buddy_system_memory::get_block_status(void * current_block) const
{
    return *reinterpret_cast<unsigned char*>(current_block) & 1;
}

void * buddy_system_memory::get_buddies(void * current_block) const
{
...
void * buddy_system_memory::get_start_allocated_memory_address() const
{
    return reinterpret_cast<void*>(reinterpret_cast<unsigned char*>(_allocated_memory) + get_allocator_service_block_size());
}

void * buddy_system_memory::get_address_relative_to_allocator(void * current_block_address) const
{
...
void buddy_system_memory::deallocate(void * target_to_dealloc) const
{
