...
memory* memory_simple::get_allocator() const
{
    return nullptr;
}
...
void* const memory_simple::allocate(size_t target_size) const
{
    try
    {
        ...
        *block_size = target_size;
        ...
        this->trace_with_guard("[GLOBAL HEAP ALLOCATOR] Memory allocated at address: " + address_to_string(block_size + 1));
...
void memory_simple::deallocate(void * target_to_dealloc) const
{
    ...
    this->trace_with_guard("[GLOBAL HEAP ALLOCATOR] Deallocate at address: " + address_to_string(target_to_dealloc));
}